                    ...
                    <form method="POST" action="/checkout/procesar" id="form-checkout">
                        ...
                        <div class="form-row">
                            <div>
                                ...
                                <input type="text" id="ciudad_departamento" name="ciudad_departamento" placeholder="Ej: Bogotá" required>
                            </div>
                            <div>
                                <label class="form-label">Barrio / Sector:</label>
                                <input type="text" id="barrio_sector" name="barrio_sector" placeholder="Ej: Chapinero" required>
                            </div>
                        </div>

                        <label class="form-label">Dirección Completa (Calle, Carrera, Número):</label>
                        <input type="text" id="direccion" name="direccion" placeholder="Ej: Calle 65 # 9-15" required>
...
<script>
    ...
    // FUNCIÓN INTELIGENTE: Filtra caracteres extraños para que la API entienda cualquier dirección
    function buscarDireccionEscrita() {
        ...
        // --- TRUCO DE LIMPIEZA DE NOMENCLATURA ---
        // 1. Quitamos los símbolos '#' que rompen las URLs de las APIs de mapas
        direccion = direccion.replace(/#/g, "");

        // 2. Si el usuario escribe cosas como "Apto", "Torre", "Interior", cortamos la cadena 
        // para que busque solo la calle y número principal.
        let direccionFiltrada = direccion.split(/(apto|apartamento|torre|int|interior|casa|piso)/i)[0].trim();

        // Armamos la consulta final forzando tu país (Colombia) para acotar la búsqueda
        const consultaCompleta = `${direccionFiltrada}, ${barrio}, ${ciudad}, Colombia`;

        // Consultamos la API aplicando el filtro de país "co" para evitar que busque fuera
        fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(consultaCompleta)}&countrycodes=co&limit=1`)
            ...
            .then(data => {
                if (data.length > 0) {
                    ...
                    // Movemos el mapa de inmediato al lugar encontrado con un zoom preciso
                    map.setView([latActual, lngActual], 16); 
                    marker.setLatLng([latActual, lngActual]);
                } else {
                    // SEGUNDO INTENTO: Si todavía no lo encuentra, busca solo la vía y la ciudad
                    // Rompe en los guiones comunes de las esquinas (ej: Calle 65 9-15 -> busca Calle 65 9)
                    ...
                    fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(consultaAlterna)}&countrycodes=co&limit=1`)
                        ...
                        .then(dataAlt => {
                            if (dataAlt.length > 0) {
                                ...
                                alert("No pudimos ubicar el punto exacto. Mueve el marcador manual en el mapa para confirmar.");
                            }
                        });
                }
            })
    ...
    // Funciones de arrastre libre del pin por el usuario
